...
class Player
{
    ...
    static void Main(string[] args)
    {
        ...
        while (true)
        {
            ...
            if (roundType == -1) Console.WriteLine("DEADPOOL");
            ...
            else
            {
                for (int n = 0; n < roundType; n++)
                {
                    ...
                    if (currentHero.heroType.Equals("IRONMAN"))
                    {
                        ...
                        else if (target != null && currentHero.mana >= 50 && target.Distance(currentHero) < 250 && currentHero.countDown2 == 0)
                        {
                            Console.WriteLine("BURNING " + target.X + " " + target.Y);
                        }
                        else if(target != null &&  currentHero.mana >16 && currentHero.attackRange>200)
                        {
                            Console.WriteLine("BLINK " + target.X + " " + target.Y);
                        }
                        else
                        {
                            HandleHero(units, currentHero);
                    ...
                    else if (currentHero.heroType.Equals("VALKYIRE"))
                    {
                        ...
                        if( currentHero.mana >= 50){
                            Console.WriteLine("POWERUP");
                        }
                        else
                        if (target != null && currentHero.mana >= 35 && target.Distance(currentHero) < 250 && currentHero.countDown1 == 0)
                        {
                            Console.WriteLine("JUMP " + target.X+ " "+target.Y);
                        }
                        else 
                    ...
                    else if (currentHero.heroType.Equals("DEADPOOL"))
                    {
                        var target = units.OfType<Unit>().FirstOrDefault(u => u.team == 1 - myTeam);
                        
                        if( target != null && currentHero.mana >= 50  && target.Distance(currentHero) < 200){
                            Console.WriteLine("WIRE "+target.X+ " "+target.Y);
                        }
                        else
                        if (target != null && currentHero.mana >= 40  && target.Distance(currentHero) < 350 )
                        {
                            Console.WriteLine("COUNTER");
                        }
                        else 
                        if (target!=null && currentHero.mana >= 30)
                        {
                            Console.WriteLine("SPEARFLIP " + target.unitId);
                        }
                        else
